    ...
    <div class="panel-body">
        ...
        <div class="pull-right form-inline">
            ...
            <button type="button" class="btn btn-sm btn-primary"
                onclick="tableToExcel('table-berita','data','Data Berita')"><i class="fa fa-file-excel-o"></i>
                Excel</button>
        </div>
        <div class="table-responsive">
            <div class="row">
                <table ng-table="tableList" id="table-berita" show-filter="false" class="table table-hover"
                    style="white-space: nowrap;">
                    <tr ng-repeat="(k,v) in $data|orderBy:'+id_berita'" class="pointer" ng-click="read(v.id_berita)">
                        <td title="'ID'" filter="{id_berita: 'text'}" sortable="'id_berita'">@{{v.id_berita}}</td>
                        <td title="'Judul'" filter="{judul: 'text'}" sortable="'judul'">@{{v.judul}}</td>
                        <td title="'Isi Berita'" filter="{isi: 'text'}" sortable="'isi'">@{{v.isi}}</td>
...
<div class="panel panel-inverse" ng-show="f.tab=='frm'">
    ...
    <div class="panel-body frmEntry">
        <form class="form-horizontal">
            <div class="row">
                <div class="col-sm-4">
                    <label title='id_berita'>ID</label>
                    ...
                    <input type="text" ng-model="h.judul" id="h_judul" class="form-control input-sm" required
                        maxlength="100">
                </div>
                <div class="col-sm-4">
                    <label title='isi'>Isi Berita</label>
                    <textarea ng-model="h.isi" id="h_isi" class="form-control input-sm"
                        style="width: 450px; height:250px;" required></textarea>
...
<script>
    ...
    item.formData = [{ route:'berita', id: $scope.h.id_berita, path: 'berita', s: 'i', userid: $scope.f.userid}];
    },
    ...
    swal("",response,"success");
     ...
     $scope.oGallery = function(id) {
        SfService.get('upload_list',{route:'berita',id}, function(jdata) {
            console.log(jdata);

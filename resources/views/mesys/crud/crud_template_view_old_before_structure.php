...
?>
                ...
                <div class="col-sm-4">
                    ...
                    <?php if (!in_array($v->COLUMN_NAME, ['created_by', 'updated_by', 'updated_at', 'created_at', 'deleted_at', 'trash', 'isaktif', 'text', 'int'])): ?>
                        ...
                        <input type="<?=@explode(' ', $v->ELEMENT)[0]?>" ng-model="h.<?=$v->COLUMN_NAME?>" id="h_<?=$v->COLUMN_NAME?>" class="form-control input-sm" <?=$v->REQUIRED?> <?=$v->ELEMENT == 'text readonly' ? 'readonly' : ''?> <?=$v->ELEMENT == 'text number' ? 'ng-pattern="/[0-9.,]+/" format="number"' : ''?> maxlength="<?=$v->CHARACTER_MAXIMUM_LENGTH?>">
                    <?php endif?>
                    <?php endforeach?>
                </div>
                <?php endforeach?>
